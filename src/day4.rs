...
pub fn puzzle1() -> i32 {
  DATA.split("\n").fold(0, |total, line| {
    ...
    let has_repeat = line.split_whitespace().fold(false, |has_repeat, word| {
...
pub fn puzzle2() -> i32 {
  DATA.split("\n").fold(0, |total, line| {
    ...
    let has_repeat = line.split_whitespace().fold(false, |has_repeat, word| {
      ...
      if map.contains(&sorted_word) {
        return true;
      }
      map.insert(sorted_word);
      has_repeat
    });
    match has_repeat {
      true => total,
...
static DATA : &str = "una bokpr ftz ryw nau yknf fguaczl anu
