...
pub fn puzzle1() -> i32 {
  DATA.split("\n").fold(0, |total, line| {
    let min = line.split_whitespace()
      .map(|strValue| { strValue.parse::<i32>().unwrap() })
      .fold(9999999, |value, num| { if num < value { num } else { value }});

    let max = line.split_whitespace()
      .map(|strValue| { strValue.parse::<i32>().unwrap() })
      .fold(0, |value, num| { if num > value { num } else { value }});
...
pub fn puzzle2() -> i32 {
  DATA.split("\n").fold(0, |total, line| {
    let vec : Vec<i32> = line.split_whitespace().map(|strValue| { strValue.parse::<i32>().unwrap() }).collect();
//    let vec2 : Vec<i32> = vec.iter().collect();

    for item1 in vec.iter() {
      for item2 in vec.iter() {
        if item1 % item2 == 0 && item1 != item2 {
          return total + (item1 / item2);
        }
      }
    }
    panic!("Found nothing divisible.");
    0
  })
}

const DATA : &str = "1364	461	1438	1456	818	999	105	1065	314	99	1353	148	837	590	404	123
